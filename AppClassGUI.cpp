...
#pragma endregion
	...
	if (m_bGUI_Main)
	{
		...
		ImGui::Begin(sAbout.c_str(), (bool*)0, window_flags);
		{
			...
			if (m_bDebug) {
				ImGui::Text("DEBUG MODE ACTIVATED");
				ImGui::Text("Control:\n");
				ImGui::Text("M: Set speed to high\n");
				ImGui::Text("L: Nuke Framerate\n");
				ImGui::Text("K: Restore framerate\n");
				ImGui::Text("U: Turn S.O. on/off\n");
			}
			ImGui::Separator();
			...
			for (int i = 0; i < numHealthToDraw; i++)
				healthBar += "|";
			for (int i = 0; i < numHealthToNotDraw; i++)
				healthBar += "_";

			// Display the meltdown bar
